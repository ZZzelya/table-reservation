...
>>>>>>> c4a7e1c27756781840560862c554d2d2c6d3c956
</div>
...
Данный проект представляет собой REST API сервис для бронирования столиков в ресторане. Разработан в рамках лабораторной работы по дисциплине *"Программирование на языках высокого уровня"*.

**Цель работы:** Создание Spring Boot приложения с классической многослойной архитектурой (Controller → Service → Repository), реализация REST endpoints для управления сущностями, настройка статического анализатора кода Checkstyle и интеграция с SonarCloud для непрерывной оценки качества кода.

API позволяет управлять ресторанами, столиками и бронированиями, предоставляя возможность искать доступные слоты по временным параметрам. В проекте сделан упор на чистоту кода, следование стандартам Java Code Conventions и документирование API.
...
|-------|----------|----------|
| `GET` | `/api/restaurants` | Получение списка всех ресторанов |
| `GET` | `/api/restaurants/{id}` | Просмотр детальной информации о конкретном ресторане |
| `GET` | `/api/restaurants/search` | Поиск ресторанов по названию |


### 🪑 Управление столиками (`/api/tables`)
| Метод | Endpoint | Описание |
|-------|----------|----------|
| `GET` | `/api/tables` | Получение списка столиков с фильтрацией по ресторану и вместимости |

### 📅 Система бронирования (`/api/reservations`)
| Метод | Endpoint | Описание |
|-------|----------|----------|
| `GET` | `/api/reservations/available` | 🔑 **Поиск свободных столиков** (основная бизнес-логика) |
| `POST` | `/api/reservations` | Создание новой брони |
| `DELETE` | `/api/reservations/{id}` | Отмена существующей брони |
| `GET` | `/api/reservations/customer/{phone}` | Просмотр всех броней клиента |

**Ключевая функция:** Поиск свободных столиков в конкретном ресторане на определенную дату и время с учетом вместимости компании и длительности брони.
...
| **Фреймворк** | ![Spring Boot](https://img.shields.io/badge/Spring%20Boot-3.2-6DB33F?style=flat-square&logo=spring&logoColor=white) |
...
| **Хранение данных** | In-memory коллекции |
...
- **Обработка данных:** Ручное управление коллекциями в памяти (имитация работы Repository слоя)
- **Качество кода:**
  - **Checkstyle:** Настроен на проверку стиля кода по правилам Oracle/Google Java Style
  - **SonarCloud:** Непрерывный анализ кода на наличие багов, уязвимостей и "запахов"
